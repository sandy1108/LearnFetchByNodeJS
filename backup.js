...
function startRequest(x) {
	...
	function(res) {
		...
		function() {
			...
			$('a.xst').each(function(index, element){
				console.log("index: "+index, "title: "+$(this).text(), "link: " + $(this).attr('href'));
			});
...
function savedContent($, news_title) {
	$('.article-content p').each(function(index, item) {
		...
		if (y == '') {
			...
			fs.appendFile('./data/' + news_title + '.txt', x, 'utf-8',
			function(err) {
				if (err) {
					console.log(err);
				}
